...
fn get_live_core_journal_events(locale: Option<String>) -> Result<Vec<Value>, String> {
    ...
    let events = lines[start..]
        ...
        .filter(|event| matches!(
            ...
            Some("Commander" | "LoadGame" | "Loadout" | "FSDJump" | "CarrierJump" | "FSSDiscoveryScan" | "FSSBodySignals" | "SAASignalsFound" | "Scan" | "ScanOrganic" | "SupercruiseEntry" | "SupercruiseExit" | "Docked" | "Undocked" | "Touchdown" | "Liftoff" | "Location")
        ))
        .collect();
    *cursor = Some(AnnaLiveJournalCursor { path, line_number: lines.len() });
    Ok(events)
