...
function requireEnv(name: string): string {
  ...
  if (!value) {
    ...
    console.error(
      "For local development, copy packages/discord-bot/.env.example to packages/discord-bot/.env."
    );
    console.error(
      "Bot startup was skipped because credentials are missing."
    );
...
export const DISCORD_WELCOME_CHANNEL_ID = requireEnv(
  "DISCORD_WELCOME_CHANNEL_ID"
);

export const DISCORD_RULES_CHANNEL_ID = requireEnv(
  "DISCORD_RULES_CHANNEL_ID"
);

export const DISCORD_COMMANDER_ROLE_ID = requireEnv(
