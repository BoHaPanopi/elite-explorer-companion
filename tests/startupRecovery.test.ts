...
test("startup routing is finalized after the journal commander state commits and before greeting", () => {
  ...
  assert.match(app, /for \(const event of coreJournalEvents\) \{[\s\S]*bridge\?\.ingestRuntimeDecisions\([\s\S]*corePredictionBaselineReady\.current \? annaEvidenceService\.predictions\(\) : null,[\s\S]*runtimeResult\?\.explorationDecisions[\s\S]*\}[\s\S]*setCoreCommander\(journalCommander\)/);
