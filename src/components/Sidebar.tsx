...
export default function Sidebar({ page, setPage }: SidebarProps) {
  ...
  return <aside className={`sidebar sidebar--${page}`}><nav>{items.map(([id, label]) => <button key={id} type="button" className={page === id ? "active" : ""} onClick={() => setPage(id)}>{label}</button>)}</nav></aside>;
}
